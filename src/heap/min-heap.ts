...
export class MinHeap<T = number> {
  ...
  protected compare = (v1: T, v2: T): boolean =>
  ...
  private getChildIndex(
    ...
    return [
      leftChildIndex > maxIndex ? undefined : leftChildIndex,
      rightChildIndex > maxIndex ? undefined : rightChildIndex,
    ];
  }
  ...
  private heapifyUp(startIndex: number = this.heap.length - 1) {
    ...
    while (
      parentIndex !== null &&
      !this.compare(heap[parentIndex], heap[currentIndex])
    ) {
  ...
  private heapifyDown(startIndex = 0) {
    ...
    while (nextIndex) {
      const [leftChildIndex, rightChildIndex] =
        this.getChildIndex(currentIndex);

      // 右边最小，左边最大
      if (
        leftChildIndex &&
        rightChildIndex &&
        this.compare(heap[rightChildIndex], heap[leftChildIndex])
      ) {
  ...
  find(value: T, comparator?: (v1: T, v2: T) => 0 | -1 | 1): number[] {
    ...
    const compareFn =
      comparator ??
      ((v1: unknown, v2: unknown) => (v1 as number) - (v2 as number));
